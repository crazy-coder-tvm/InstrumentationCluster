...
void QcGaugeWidget::paintEvent(QPaintEvent *)
{
...
void QcLabelItem::draw(QPainter *painter)
{
    ...
    //QFont font("Ubuntu condensed", r / 10.0, QFont::Bold);
    QFont font("Pirulen", r/15.0, QFont::Thin, true);
...
QPainterPath QcColorBand::createSubBand(float from, float sweep)
{
    ...
    path.arcTo(tmpRect, 180 + from, 360);
    return path;
}
...
void QcColorBand::draw(QPainter *painter)
{
    ...
    pen.setWidthF(r/70);
    painter->setBrush(Qt::NoBrush);
    float offset = getDegFromValue(0);
...
void QcDegreesItem::draw(QPainter *painter)
{
    ...
    float r = getRadius(tmpRect);
    for (float val = mMinValue; val <= mMaxValue; val++) {
        if(mMaxValue <= 20) {
            if (!(fmodf(val , 1) == 0))
            {
                val += 1;
...
void QcDegreesItem::print_degree(float val  , float r , QRectF tmpRect , QPainter *painter)
{
    ...
    //pt = path.pointAtPercent(0.02);
    QPointF newPt;
    QPen pen;
    if(mMaxValue > 20) {
        if((int)val % 20 == 0) {
            newPt = path.pointAtPercent(0.07);
            pen.setWidthF(r / 50.0);
            pen.setColor(Qt::white);
        } else if((int)val % 10 == 0) {
            newPt = path.pointAtPercent(0.03);
            pen.setWidthF(r / 50.0);
            pen.setColor(Qt::darkGray);
            //return;
        } else {
            // Sub degree
//            newPt = path.pointAtPercent(0.01);
//            pen.setWidthF(r / 75.0);
//            pen.setColor(Qt::darkGray);
            return;
        }
        ...
        if((int)val % 1 == 0) {
            ...
            // Sub degree
            newPt = path.pointAtPercent(0.05);
            pen.setWidthF(r / 50.0);
            pen.setColor(Qt::gray);
        }
    }

    if(val >  mMaxValue * 0.83) {
        pen.setColor(Qt::red);
    }
...
void QcValuesItem::draw(QPainter*painter)
{
    ...
    //QFont font("Ubuntu condensed", 0, QFont::Bold);
    QFont font("Pirulen", 0, QFont::Thin, true);
